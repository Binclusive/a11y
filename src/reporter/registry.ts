 ...
 * (the generic stdout adapter, the ≥ 2 proof), and `gitlab` (MR notes, #213). New
 * platforms register here; nothing else changes.
 *
...
export function defaultBoundAdapters(): BoundAdapter[] {
  return [bindAdapter(githubAdapter), bindAdapter(nullAdapter), bindAdapter(gitlabAdapter)];
}

/** The default registry: `github` + `null` + `gitlab`, selectable by explicit platform key. */
