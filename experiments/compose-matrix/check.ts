 ...
 *
 * Re-scans the SHA-pinned Compose corpus with the CURRENT Compose checker, diffs the
 * result against the committed baseline.json, prints every repo whose numbers moved,
 * and exits non-zero if anything did. Run it before opening a PR that touches the
 * Compose path — the TS boundary (`src/collect-kotlin.ts`) OR the out-of-process
 * Kotlin PSI engine (`kotlin/A11yKotlinScan/`, rule `compose/image-no-label`).
 *
 * The gated quantity is the FINDING stream (count + per-rule counts + the sorted
 * finding list); the Compose engine emits findings only, so there is no secondary
 * parse-outcome layer (unlike unity-matrix).
 *
 ...
 *
 * The engine must be built before this runs (`./gradlew installDist` — see README).
 *
 * Flags:
...
async function main(): Promise<void> {
  ...
  if (baseline === null) {
    console.error("No baseline.json found. Create one with `pnpm compose:matrix:baseline` first.");
    process.exit(1);
  }

  if (!skipRun) {
    console.log("Re-scanning pinned Compose corpus (clones + scans each repo — minutes)…\n");
    await runAll();
    console.log("");
  }
  ...
  console.log(`Real-world Compose deltas vs baseline (${unchanged} unchanged, ${deltas.length} moved):\n`);
