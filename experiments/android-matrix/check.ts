/**
 * check.ts — `android:matrix:check`: the Android XML real-world regression gate.
 *
 * Re-scans the SHA-pinned corpus with the CURRENT XML producer, diffs the result
 * against the committed baseline.json, prints every repo whose numbers moved, and
 * exits non-zero if anything did. Run it before opening a PR that touches the XML
 * producer (`android-xml-ast.ts`, `android-xml-rules.ts`, `collect-android-xml.ts`).
 *
 * A non-zero exit is NOT "your change is wrong" — it is "your change moved real-world
 * behavior, look at the delta." If the movement is intended (a rule now catches a real
 * bug, or a fixed false positive disappears), re-bless with `pnpm android:matrix:baseline`
 * and commit the updated baseline.json alongside your change so the shift is visible in
 * review.
 *
 * Flags:
...
function fmtDelta(d: RepoDelta): string {
  ...
  if (d.filesScanned) span("layouts", d.filesScanned);
...
async function main(): Promise<void> {
  ...
  console.log(`Real-world Android XML deltas vs baseline (${unchanged} unchanged, ${deltas.length} moved):\n`);
