/**
 * baseline.ts — the Android XML real-world regression baseline: a committed snapshot
 * of the producer's results across the SHA-pinned corpus, plus the diff that turns
 * the corpus into a regression gate. Android analog of `experiments/unity-matrix/baseline.ts`.
 *
 * The corpus (manifest.json) is SHA-pinned, so the only thing that can move these
 * numbers is the XML producer's own code (`android-xml-ast.ts`, `android-xml-rules.ts`,
 * `collect-android-xml.ts`). Each per-repo result is distilled into a compact,
 * deterministic record and committed as `baseline.json` — every change that shifts
 * real-world behavior shows up as an edit to that file in the PR.
 *
 * PRIMARY gated quantity: the FINDING stream (`findingsCount` / `byRule` / `findings`).
 * `filesScanned` + `parseErrors` are a SECONDARY assertion, diffed too.
 *
...
/** The compact, committed snapshot of one repo's scan. `findings` is the sorted full
 * list so a moved/added/removed finding is a line-level, reviewable diff. */
...
export interface RepoDelta {
  ...
  readonly filesScanned?: { before: number; after: number };
...
/**
 * Compare a current snapshot against the committed baseline. ANY movement — primary
 * (findings count, per-rule counts) or secondary (files scanned, parse errors, error
 * transition) — is surfaced. The caller decides intended-vs-regression and re-blesses;
 * the gate's job is only to make movement impossible to miss.
 */
export function diffBaseline(current: Baseline, baseline: Baseline): DiffResult {
  ...
  for (const repo of [...repos].sort()) {
    ...
    deltas.push({
      ...
      ...(filesMoved ? { filesScanned: { before: base.filesScanned, after: cur.filesScanned } } : {}),
