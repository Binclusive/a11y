...
| Buildkite | [`buildkite/pipeline.yml`](buildkite/pipeline.yml) | Native `buildkite` reporter posts a **build annotation** (`A11Y_PLATFORM=buildkite`) via `buildkite-agent annotate` — needs `mount-buildkite-agent: true` so the binary + token reach the container (no API token to manage). No fixed clone depth (agent-dependent) — the config unshallows before the diff. No managed secret store: `B8E_TOKEN` comes from an agent env/pre-command hook. |
