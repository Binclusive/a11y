...
| `max-violations` | *(none — no volume gate)* | Optional. Fail the run when the number of non-baselined findings **exceeds** this count. A volume cap, independent of `enforce` and `fail-on` — set it to today's count on a repo with a backlog and the build reds only when the total grows. |
