...
```bash
pnpm scan:url https://www.havas.net
```

```
a11y-checker — rendering https://www.havas.net and running axe-core
...
`havas.net` is a real customer — ASP.NET Razor + jQuery, not a line of React. The
...
```bash
pnpm scan:url https://www.havas.net     # a deployed site
